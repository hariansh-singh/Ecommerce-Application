...
export class ShopComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    ...
    this.applyFilters(); // Ensures filtering updates dynamically
  });
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['searchQuery']) {
      this.applyFilters(); // Automatically trigger filtering on change
    }
  }
    ...
    this.productService.getAllProducts().subscribe({
      next: (response: any) => {
        ...
        if (Array.isArray(this.products)) {
          ...
          console.error(
            'Products is not an array after processing:',
            this.products
          );
          this.products = [];
          this.categories = [];
  ...
  extractCategories(): void {
    ...
    if (!Array.isArray(this.products)) {
      console.error(
        'Products is not an array in extractCategories:',
        this.products
      );
      this.categories = [];
      return;
  ...
  applyFilters(): void {
    ...
    if (!Array.isArray(this.products) || !this.products.length) {
      console.error('Products is not a valid array:', this.products);
      this.filteredProducts = [];
      this.totalPages = 0;
      return;
    }
    ...
    // Search filter
    const term = (this.searchQuery || this.searchTerm || '')
      .toLowerCase()
      .trim();
    if (term) {
    ...
    // Sorting logic
    const sortingMap: Record<string, (a: any, b: any) => number> = {
    ...
    if (this.sortOption && sortingMap[this.sortOption]) {
      filtered.sort(sortingMap[this.sortOption]);
    }
  ...
  getImageUrl(product: any): string {
    ...
    return `https://via.placeholder.com/300x200/cccccc/333333?text=${encodeURIComponent(
