...
export class ViewCartComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (selectedAddress) {
      ...
      this.addressService
        .getDefaultAddress(this.decodedToken.CustomerId)
        .subscribe({
          next: (defaultAddress) => {
            if (defaultAddress) {
        ...
        if (Array.isArray(response)) {
          this.cartItems = response.map((item) => ({
            ...
            products: {
              ...item.products,
              imageUrl:
                item.products.imageUrl || 'https://via.placeholder.com/150', // Using an online placeholder
            },
          }));
  ...
  placeOrder(): void {
    ...
    if (!this.shippingAddress) {
      this.showWarningToast(
        'Please provide an address before placing an order.'
      );
      return;
    }
      ...
      .subscribe({
        next: (response: any) => {
          ...
          if (response.err === 0 && response.data) {
            // Send invoice with real order data from API
            try {
              this.invoiceService.uploadInvoice(
                response.data, // Real order data from backend
                this.decodedToken.Name ||
                  this.decodedToken.Username ||
                  'hariansh-singh', // Using your login
                this.decodedToken.Email ||
                  this.decodedToken.email ||
                  'hariansh@example.com' // Fallback email
              );
            ...
            console.warn('Order placed but no order data returned for invoice');
          }
  ...
  applyPromoCode(): void {
    ...
    switch (code) {
      ...
      case 'FLAT500':
        ...
        break;
      case 'SAVE20':
        this.discount = this.totalPrice * 0.2; // 20% off
        break;
      case 'SUMMER150':
        this.discount = 150;
        break;
